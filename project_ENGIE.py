...
def clean_scada(scada_file: str | Path) -> pd.DataFrame:
    ...
    scada_df["Date_time"] = pd.to_datetime(
        scada_df["Date_time"], utc=True).dt.tz_localize(None)
    ...
    scada_df = scada_df[(scada_df["Ot_avg"] >= -15.0) &
                        (scada_df["Ot_avg"] <= 45.0)]
    ...
    sensor_cols = ["Ba_avg", "P_avg", "Ws_avg",
    ...
    for t_id in turbine_id_list:
        ...
        ix_flag = filters.unresponsive_flag(
            scada_df.loc[ix_turbine], 3, col=["Va_avg"])
        scada_df.loc[ix_flag.loc[ix_flag["Va_avg"]].index,
                     sensor_cols] = np.nan

        # Cancel out the temperature readings where the value repeats more than 20 times in a row
        ix_flag = filters.unresponsive_flag(
            scada_df.loc[ix_turbine], 20, col=["Ot_avg"])
    ...
    scada_df.loc[ix_gt_180,
                 "Ba_avg"] = scada_df.loc[ix_gt_180, "Ba_avg"] - 360.0

    logger.info("Calculating energy production")
    scada_df["energy_kwh"] = un.convert_power_to_energy(
        scada_df.P_avg * 1000, scada_freq) / 1000
...
def load_cleansed_data(path: str | Path, return_value="plantdata") -> PlantData:
    ...
    if return_value == "dataframes":
        ...
        engie_plantdata = PlantData(
            # Choosing a random type that doesn't fail validation
        ...
        raise ValueError(
            "`return_value` must be one of 'plantdata' or 'dataframes'.")
...
def prepare(
    ...
    meter_df.drop(["time_utc", "availability_kwh",
    ...
    # Make another copy for modifying the curtailment data
    curtail_df = meter_curtail_df.copy()

    # Create datetime field with a UTC base
    curtail_df["time"] = pd.to_datetime(
        curtail_df.time_utc).dt.tz_localize(None)
    ...
    ###################
    logger.info(
        "Reading in the reanalysis data and calculating the extra fields")
    ...
    reanalysis_era5_df = reanalysis_era5_df.loc[:, ~
                                                reanalysis_era5_df.columns.duplicated()].copy()
    ...
    reanalysis_era5_df = reanalysis_era5_df.set_index(
        pd.DatetimeIndex(reanalysis_era5_df.datetime))
    ...
    if return_value == "dataframes":
        ...
        engie_plantdata = PlantData(
            ...
            curtail=curtail_df,
            asset=asset_df,
            reanalysis={"era5": reanalysis_era5_df,
                        "merra2": reanalysis_merra2_df},
        )
        return engie_plantdata
    else:
        raise ValueError(
